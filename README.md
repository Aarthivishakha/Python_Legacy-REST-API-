# Legacy REST API / Microservice (Python 3.16 development)

A framework-free JSON microservice built on the Python 3.16 WSGI standard
...
```bash
docker build -t legacy-rest-api .
docker run --rm -p 8080:8080 legacy-rest-api
```
...
- `pyproject.toml`: package, pytest, coverage, and Ruff configuration.
- `requirements-dev.txt`: native test, coverage, and lint dependencies.
- `Dockerfile`: reproducible non-root service container.
- `.github/workflows/ci.yml`: native Python 3.16 and isolated analyzer jobs.

Current analyzers run separately under Python 3.10+; application source remains
compatible with Python 3.16.

Python 3.16 is in early development. Native CI uses `3.16-dev`; because no
official Python 3.16 image exists, the multi-stage Dockerfile builds current
CPython development source. Ruff uses its newest available `py315` grammar,
and analyzer execution stays isolated on Python 3.10+.
